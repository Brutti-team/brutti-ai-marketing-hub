        ...
        {activeView === "overview" && (
          <>
            ...
            <section className="metric-grid" aria-label="Facebook workflow summary">
              <article className="metric-card"><span className="metric-icon moss">F</span><div><p>Verified activity</p><strong>{facebookAnalytics.uniquePosts.toLocaleString()}</strong><small>Estimated unique Facebook posts</small></div></article>
              <article className="metric-card"><span className="metric-icon amber">◷</span><div><p>Review queue</p><strong>{reviewCount}</strong><small>Generated drafts awaiting review</small></div></article>
          ...
          <>
            <section className="panel">
              ...
              <div className="analytics-summary">
                <div><strong>{facebookAnalytics.groupPostRows.toLocaleString()}</strong><span>Group-post rows</span></div><div><strong>{facebookAnalytics.uniquePosts.toLocaleString()}</strong><span>Estimated unique posts</span></div><div><strong>{facebookAnalytics.groups}</strong><span>Groups reached</span></div><div><strong>{facebookAnalytics.productMentions}</strong><span>Product / price mentions</span></div>
              </div>
              <div className="report-grid">
                <article className="report-block"><h4>Monthly activity</h4><div className="bar-list">{facebookAnalytics.monthly.map((item) => <div className="bar-row" key={item.month}><span>{item.month}</span><i style={{ width: `${Math.max(4, (item.rows / 687) * 100)}%` }} /><strong>{item.rows}</strong></div>)}</div></article>
            ...
            <section className="panel section-gap limitation-panel"><h3>Data limitations</h3><p>The Facebook export does not contain verified reach, views, reactions, shares, follower growth or conversion metrics. Therefore this website does not invent performance figures or claim which post was viral/best-performing.</p></section>
          </>
        )}
