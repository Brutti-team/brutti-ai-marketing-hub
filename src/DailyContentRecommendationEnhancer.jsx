...
function card(idea, index, snapshot) {
  ...
  return '<article class="daily-recommendation-card" style="padding:18px;border:1px solid #b9d0c4;border-radius:18px;background:#f2f7f3;color:#173c30;box-shadow:0 8px 24px rgba(20,74,58,.10);display:flex;flex-direction:column;gap:12px">'
    + '<div style="display:flex;justify-content:space-between;gap:8px"><span style="font-size:10px;font-weight:800;letter-spacing:.1em;color:#17634d">IDEA 0' + (index + 1) + '</span><span style="font-size:10px;font-weight:700;padding:5px 9px;border-radius:999px;background:#dbeee3;color:#14503e">Confidence: ' + slot.confidence + '</span></div>'
    + '<div><h3 style="margin:0 0 6px;font-size:18px;line-height:1.28;color:#173c30">' + idea.title + '</h3><p style="margin:0;font-size:14px;line-height:1.5;color:#315447">' + idea.idea + '</p></div>'
    + '<div style="padding:11px;border-radius:12px;background:#dfece4;color:#21463a"><strong style="font-size:12px">Brutti Soul Master reason</strong><p style="margin:5px 0 0;font-size:12px;line-height:1.48;color:#315447">' + idea.reason + '</p></div>'
    + '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;font-size:12px;color:#21463a"><div><span style="display:block;font-size:9px;letter-spacing:.04em;color:#557468">BEST TIME TO POST</span><strong>' + slot.label + '</strong></div><div><span style="display:block;font-size:9px;letter-spacing:.04em;color:#557468">POSTING TIME</span><strong>' + slot.time + '</strong></div><div><span style="display:block;font-size:9px;letter-spacing:.04em;color:#557468">TARGET</span><strong>' + idea.target + '</strong></div><div><span style="display:block;font-size:9px;letter-spacing:.04em;color:#557468">OBJECTIVE</span><strong>' + idea.objective + '</strong></div></div>'
    + '<div><span style="display:block;font-size:9px;letter-spacing:.04em;color:#557468">SUGGESTED FORMAT</span><strong>' + idea.suggested + '</strong></div><small style="font-size:11px;line-height:1.4;color:#557468">' + context(index, snapshot) + '</small>'
    + '<div style="display:flex;gap:9px;flex-wrap:wrap;margin-top:auto"><button type="button" class="btn-primary daily-use-idea" style="background:#17634d;color:#fff;border-color:#17634d;font-size:13px;padding:9px 12px" data-index="' + index + '">Use This Idea</button><button type="button" class="btn-secondary daily-add-planner" style="background:#fff;color:#17634d;border:1px solid #17634d;font-size:13px;padding:9px 12px" data-index="' + index + '">Add to Planner</button></div></article>'
}
