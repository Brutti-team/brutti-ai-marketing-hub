...
function dynamicUseCase(subject, profile = {}, variation = 0) {
  ...
  if (/graduat|memento|kenangan|sijil|display.*kenangan|kenangan.*display/i.test(text)) return ['Boleh dipamerkan sebagai kenangan yang tetap ada selepas majlis selesai.', 'Display macam ni senang jadi sebahagian daripada ruang tanpa hilang cerita di sebaliknya.', 'Bila diletak di ruang yang sesuai, kenangan graduasi tu terus terasa.'][variation % 3]
  if (/event|setup|indoor|outdoor|gula|jajan/i.test(text)) return ['Setup dia boleh ikut ruang yang kamu ada.', 'Boleh bergerak dari satu setup ke setup lain tanpa hilang fungsi dia.', 'Bila masuk dalam event, susunan barang pun lebih senang diatur.'][variation % 3]
  if (/display|pamer|hias|decoration/i.test(text)) return ['Boleh dipamerkan ikut ruang yang kamu ada.', 'Rupa dia senang menyatu dengan ruang tanpa mengganggu fungsi utama.', 'Bila diletak pada tempat yang sesuai, detail dia lebih senang nampak.'][variation % 3]
...
function categoryAngle(form = {}, style = {}, reference = null, variation = 0) {
  ...
  if (referenceSequence.includes('brand-reflection') || style.firstPerson) {
    ...
    if (/graduat|memento|kenangan|sijil/.test(facts)) return ['Benda macam ni bukan sekadar display — ada kenangan yang dia simpan.', 'Bila diletak dalam ruang, memento ni terus bawa balik cerita hari graduasi.', 'Detail kecil macam ni yang buat satu hadiah rasa lebih personal.'][variation % 3]
