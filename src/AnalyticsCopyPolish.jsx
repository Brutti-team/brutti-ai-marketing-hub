...
function renderMeta(page, payload) {
  ...
  const totals = posts.reduce((sum, post) => ({
    reach: sum.reach + Number(post.reach || 0),
    views: sum.views + Number(post.views || 0),
    reactions: sum.reactions + Number(post.reactions || 0),
    comments: sum.comments + Number(post.comments || 0),
    shares: sum.shares + Number(post.shares || 0),
  }), { reach: 0, views: 0, reactions: 0, comments: 0, shares: 0 })
  const section = document.createElement('section')
  section.className = 'panel meta-kpi-panel'
  ...
  const chip = page.querySelector('.status-chip')
  if (chip) chip.textContent = 'Meta KPIs connected'
}
...
export default function AnalyticsCopyPolish() {
  useEffect(() => {
    ...
    const sync = () => {
      ...
      page.querySelectorAll('.status-chip').forEach((chip) => replaceExact(chip, 'Meta KPI excluded', 'Meta KPIs excluded'))
      if (!loaded) {
        loaded = true
        fetch(endpoint, { cache: 'no-store' }).then((response) => response.ok ? response.json() : null).then((payload) => payload && renderMeta(page, payload)).catch(() => {})
      }
    }
    sync()
