...
const catalogVisualEntries = Object.keys(catalogVisualByProduct)
  .filter((name, index, names) => names.indexOf(name) === index)
  .map((name) => ({ name, imageUrl: catalogVisualFor(name) }))
...
function ProductLibrary({ onUseProduct, productData, workspaceActive, notionActive, onSyncNotion, syncing, onSaveReference, onDeleteReference, toast }) {
  ...
  return (
    <div className="page">
      ...
      <section className="panel catalog-visual-panel"><div className="panel-heading"><div><span className="eyebrow">BRUTTI CATALOG VISUALS</span><h3>Gambar daripada PDF katalog</h3><p className="settings-copy">Visual katalog ini boleh digunakan sebagai rujukan produk, berasingan daripada padanan Meta.</p></div><span className="source-count">{catalogVisualEntries.length} visual</span></div><div className="catalog-visual-grid">{catalogVisualEntries.map((entry) => <article key={entry.name}><img src={entry.imageUrl} alt={`${entry.name} catalog visual`}/><div><strong>{entry.name}</strong><button className="text-button" type="button" onClick={() => onUseProduct({ name: entry.name, imageUrl: entry.imageUrl })}>Use visual <Icon name="arrow" size={15}/></button></div></article>)}</div></section>
