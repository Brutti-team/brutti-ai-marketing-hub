...
import {
  ...
  suggestProductImageMatches,
  confirmProductImageMatch,
  setWorkspaceKey,
...
const catalogVisualFor = (name = '') => {
  ...
  // Keep the Pages sub-path explicit: catalog files are served from the app's
  // public folder and must not resolve against the domain root.
...
function buildSmartDraft(form, mode = 'balanced', variation = 0) {
  ...
  const conciseRequest = form.length === 'short' || /(?:\b4\s*baris\b|\bempat\s*baris\b|pendek|ringkas|short caption)/i.test(form.brief)
...
function GeneratorForm({ form, setForm, onGenerate, output, onOutputChange, saveDraft, workspaceActive, toast, productOptions }) {
  ...
  return (
    <div className={`generator-layout ${output ? 'has-output' : ''}`}>
      ...
      <div className={`generator-output ${output ? 'has-output' : ''}`} style={output ? undefined : { display: 'none' }}>
        ...
        {output ? <><label className="output-editor-label">Editable Facebook caption · {form.length === 'full' ? '7–13' : '4–5'} content lines<textarea value={output} onChange={(event) => onOutputChange(event.target.value)} rows="18"/></label><section className="smart-rewrite-panel"><div className="smart-rewrite-head"><div><span className="eyebrow">FREE SMART REWRITE</span><strong>Asah caption ikut suara Brutti</strong></div><span>Brutti Sabahan Casual · No API</span></div><div className="rewrite-actions"><button className={rewriteMode === 'engaging' ? 'active' : ''} onClick={() => rewrite('engaging')}>More engaging</button><button className={rewriteMode === 'casual' ? 'active' : ''} onClick={() => rewrite('casual')}>More casual</button><button className={rewriteMode === 'professional' ? 'active' : ''} onClick={() => rewrite('professional')}>More professional</button><button className={rewriteMode === 'shorten' ? 'active' : ''} onClick={() => rewrite('shorten')}>Shorter lines</button><button className={rewriteMode === 'hook' ? 'active' : ''} onClick={() => rewrite('hook')}>New hook</button><button className={rewriteMode === 'cta' ? 'active' : ''} onClick={() => rewrite('cta')}>New CTA</button><button className={rewriteMode === 'hashtags' ? 'active' : ''} onClick={() => rewrite('hashtags')}>Refresh hashtags</button></div><div className="variation-row"><span>Brutti Sabahan variations</span><button onClick={() => rewrite('balanced', (variation + 1) % 3)}>Regenerate</button>{[0,1,2].map((index) => <button className={rewriteMode === 'balanced' && variation === index ? 'active' : ''} key={index} onClick={() => rewrite('balanced', index)}>Version {index + 1}</button>)}</div></section><div className="output-actions assist-actions"><button className="button secondary" onClick={() => navigator.clipboard?.writeText(output)}>Copy caption</button><button className="button primary" onClick={saveDraft}>Save as draft</button></div><div className="ai-checks system-copy-hidden">{checks.map((check) => <span className={check.pass ? 'pass' : check.review ? 'review' : 'flag'} key={check.label}><Icon name={check.pass ? 'check' : 'alert'}/>{check.label}</span>)}</div><p className="save-location system-copy-hidden">{workspaceActive ? 'Saving writes this draft to the BRUTTI Google Sheet.' : 'Saving keeps this draft in the current browser until Google is connected.'}</p></> : <div className="empty-output"><div className="sparkle-ring"><Icon name="sparkles" size={28}/></div><h3>Free Smart Rewrite is ready.</h3><p>Add verified facts to generate a 7–13-line BRUTTI Facebook caption, then refine its hook, tone, CTA and hashtags without leaving this website.</p></div>}
      </div>
    </div>
  )
}
...
function ProductLibrary({ onUseProduct, productData, workspaceActive, notionActive, onSyncNotion, syncing, onSaveReference, onDeleteReference, toast }) {
  ...
  const loadImageSuggestions = async () => {
    ...
    try { const result = await suggestProductImageMatches(); setImageSuggestions(result.suggestions || []); toast('Cadangan gambar Meta sudah dimuatkan.') } catch (error) { toast(error.message) } finally { setLoadingSuggestions(false) }
  }
  const confirmImage = async (productId, postId) => {
  ...
  const handleImage = (event) => {
    ...
    if (!file.type.startsWith('image/') || file.size > 1200000) {
      setImageError('Use an image file up to 1.2 MB.')
      event.target.value = ''
      return
    }
    const reader = new FileReader()
    reader.onload = () => {
      setReference((current) => ({ ...current, imageDataUrl: String(reader.result || ''), imageName: file.name }))
      setImageError('')
    }
...
function Analytics({ content, plans, productData, integrations, workspaceActive, toast }) {
  ...
  return (
    <div className="page">
      ...
      <section className="panel meta-sync-summary" aria-label="Meta sync summary"><div className="panel-heading"><div><span className="eyebrow">SYNC STATUS</span><h3>Data terakhir diselaraskan</h3></div><span className="verified-label"><Icon name="check"/>Read-only source</span></div><div className="meta-sync-grid"><article><span>Last sync</span><strong>{syncTime}</strong></article><article><span>Posts read</span><strong>{totalSynced.toLocaleString('en-MY')}</strong></article><article className={`platform-status ${platformStatus(facebookCount) === 'Connected' ? 'ok' : 'pending'}`}><span>Facebook Page</span><strong>{platformStatus(facebookCount)}</strong><small>{facebookCount} post{facebookCount === 1 ? '' : 's'} in snapshot</small></article><article className={`platform-status ${platformStatus(instagramCount) === 'Connected' ? 'ok' : 'pending'}`}><span>Instagram</span><strong>{platformStatus(instagramCount)}</strong><small>{instagramCount} post{instagramCount === 1 ? '' : 's'} in snapshot</small></article></div></section>
      <div className="stats-grid analytics-stats system-copy-hidden"><article className="stat-card"><div className="stat-icon file"><Icon name="file"/></div><div><span>Content records</span><strong>{content.length}</strong><small>{content.filter((item)=>item.stage==='Review').length} awaiting review</small></div></article><article className="stat-card"><div className="stat-icon calendar"><Icon name="calendar"/></div><div><span>Scheduled plans</span><strong>{scheduledPlans}</strong><small>{plans.length} total planner items</small></div></article><article className="stat-card"><div className="stat-icon image"><Icon name="image"/></div><div><span>Drafts with visual</span><strong>{withAssets}</strong><small>Drive assets attached to content</small></div></article><article className="stat-card"><div className="stat-icon check"><Icon name="check"/></div><div><span>Published records</span><strong>{published}</strong><small>Workspace publishing history</small></div></article></div>

      <div className="analytics-grid system-copy-hidden"><section className="panel activity-chart"><div className="panel-heading"><div><span className="eyebrow">CONTENT ACTIVITY</span><h3 className="system-copy-hidden">Live workflow distribution</h3></div><span className="verified-label"><Icon name="check"/>Workspace records</span></div><div className="bar-chart">{stages.map((stage) => { const count = content.filter((item) => item.stage === stage).length; return <div key={stage}><span>{stage}</span><i><b style={{width:`${Math.round((count/maxCount)*100)}%`}}/></i><strong>{count}</strong></div> })}</div></section><section className="panel insight-card system-copy-hidden"><span className="eyebrow">RULE-BASED OBSERVATION</span><h3>{content.some((item)=>item.stage==='Review') ? 'Clear the review queue before adding too many new drafts.' : 'The review queue is clear.'}</h3><p>{integrations.notion ? 'Notion planner sync is configured for shared planning records.' : 'Notion backend sync is not configured yet; Google remains the current operational source.'}</p><div className="insight-source"><Icon name="file"/><span><strong>Next data upgrade</strong><small>Verified post URL + reach + views + engagements</small></span></div></section></div>
...
function App() {
  ...
  const [generator, setGenerator] = useState({ title:'', platform:'Facebook', type:'Behind the Scenes', product:'General / No Product', language:'Bahasa Melayu', tone:'Brutti Sabahan Casual', length:'short', brief:'', includeHashtags:true, driveFileId:'', assetName:'', driveLink:'' })
  ...
  const pages = useMemo(() => ({
    ...
    brand: <BrandLibrary />,
    products: <ProductLibrary onUseProduct={useProduct} productData={allProductData} workspaceActive={workspaceActive} notionActive={integrations.notion} onSyncNotion={syncProducts} syncing={syncingProducts} onSaveReference={saveProductReference} onDeleteReference={deleteProductReference} toast={toast} />,
